...
|---|---|---|---|---|
| A1 | Operating protocol committed (`agents/PROTOCOL.md`, v2) | orchestrator | ✅ **re-verified in this repository** 2026-08-05 | **J-orchestrator-0040** · verify: `git log --oneline -- agents/PROTOCOL.md`. *Re-pointed at AUD-0001-F2: the prior citation `J-orchestrator-0001` was authored in the canonical shell and inherited here, so it attested another repository's seeding. The local re-verification lives in J-orchestrator-0040.* |
| A2 | Enforcement self-test green in CI (every rejection asserting its rule, R1–R10 + WARN-SEAL) | orchestrator | ✅ **re-verified in this repository** 2026-08-05 | **J-orchestrator-0040** (47 passed, 0 failed; 41 scenarios) · verify: `bash scripts/test_protocol.sh` locally; same script is the first step of the `journal-check` workflow on every push. *Re-pointed at AUD-0001-F2.* |
| A3 | `journal-check` CI green on the pushed branch (full-history re-verification) | orchestrator | ✅ **re-verified in this repository** 2026-08-05 | **J-orchestrator-0040**, citing **this repository's** CI run `31012592559` (branch `main`, head `0a60b2a`, conclusion success) and `31016037714` (branch `claude/project-investigation-54wqwc`, head `fe5dea7`, success) · verify: Actions tab → `journal-check`, or `bash scripts/check_journals.sh --all`. *Re-pointed at AUD-0001-F2 — the sharpest case: the prior citation `J-orchestrator-0002` describes the canonical shell's run 1, in a CI history this repository does not contain.* |
| A4 | Charters, org chart, and spawn launchers committed and mutually consistent (roster 1:1 across `ORG_CHART.md`, `agents/charters/`, `.claude/agents/`) | orchestrator | ✅ **re-verified in this repository** 2026-08-05 | **J-orchestrator-0040** · verify: `ls agents/charters .claude/agents` against the ORG_CHART roster table (9 charters, 8 launchers + the orchestrator as the main session). *Re-pointed at AUD-0001-F2.* |
| A5 | Journals seeded, append-only from birth; `agents/journals/INDEX.md` committed | orchestrator | ✅ **re-verified in this repository** 2026-08-05 | **J-orchestrator-0040** (green over all commits; volume chains verified at range head) · verify: `bash scripts/check_journals.sh --all`. *Re-pointed at AUD-0001-F2.* |
...
| A8 | **Decision**: single-branch mode (work lands on `main` directly) vs PR-flow mode (working branch + milestone PRs into `main`, R9). **Default: single-branch mode** — the orchestrator proposes it in the intake proposal and the sponsor overrides only if they want PR-flow; either way the choice rides the E0 setup, no open-ended question asked. Decide with A7 — it changes which ruleset binds what — and record the decision and the working-branch name on [`tasks/BOARD.md`](../../tasks/BOARD.md) | sponsor + orchestrator | ✅ **PR-flow mode**, working branch `claude/project-investigation-54wqwc` — 2026-08-05 | **J-orchestrator-0043** · BOARD "Branch flow (G0 A8)". *Re-pointed at AUD-0001-F4: the previously cited entry `J-orchestrator-0040` contains no reasoning about branch flow at all, so the rationale asserted here had no supporting authority. J-orchestrator-0043 states the rationale in the signer's own record and quotes what the sponsor was asked and what they answered — the answer was the single word "confirm", and that limit is recorded rather than dressed up.* |
| A9 | Auditor's G0 retro-audit of the seed commit range committed to `docs/reports/audit/` — the new org's **first spawn**, and the first proof the audit lane works | auditor | ✅ **complete — PASS WITH FINDINGS** 2026-08-05 | **J-auditor-0001** · report [`AUD-0001-g0-retro-audit.md`](../reports/audit/AUD-0001-g0-retro-audit.md) committed at `93fd657`, baseline pinned `fe5dea7` (PROTOCOL §3) · **0 CRITICAL · 4 MAJOR · 6 MINOR; no finding blocks a gate**. The auditor's own recommendation: "satisfiable by this report". Findings F1/F3/F4 and F2's local half are against the orchestrator; **F3's closure requires the auditor's re-verification in a follow-up report and is NOT closed by this row** |
