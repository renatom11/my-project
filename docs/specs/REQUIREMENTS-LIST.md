...
- **REQ-069** — **Encoding:** `8XY1` · **Name:** OR · **Effect on V\<x\>:** V\<x\> ← V\<x\> | V\<y\> · **Effect on VF:** VF ← 0 if `QUIRK_VF_RESET`, else none  
...
- **REQ-114** — **Memory observation contract** (REQ-114): the lockstep campaign compares memory as part of the architectural state, and 32768 bits cannot sensibly be carried on a port. M02 therefore SHALL declare its storage as a single unpacked array named **`mem`**, of 4096 elements of 8 bits, at the top level of module `chip8_ram`, so that a bench can read memory by one stable hierarchical path. This is the **only** internal name this specification constrains, and it is constrained precisely so that the bench depends on a specified observable rather than on an implementation-derived path.  
